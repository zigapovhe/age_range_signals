...
* **Docs**: Added a "Choosing Your Integration Level" table mapping common app shapes to plugin usage (#30), consolidated the regulatory status (Texas, Utah, Louisiana, Brazil, Australia) into a single section, documented the supported AGP/KGP range, and fixed broken table-of-contents links. Clarified why iOS never returns `supervisedApprovalDenied` (#24).
