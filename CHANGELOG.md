...
* **iOS**: Added `isEligibleForAgeFeatures()` (iOS 26.2+), an opt-in wrapper for Apple's region check and the first step in [Apple's documented flow](https://developer.apple.com/documentation/declaredagerange/requesting-people-share-their-age-range-with-your-app#Check-eligibility-for-age-related-features). `checkAgeSignals()` still does not call it internally. Throws `UnsupportedPlatformException` below iOS 26.2, on pre-26.2 SDKs and on Android. Thanks [@danielwilliamson](https://github.com/danielwilliamson) for the PR ([#36](https://github.com/zigapovhe/age_range_signals/pull/36)).
* **Docs**: Clarified that an empty regulatory feature set can still mean obligated (per [Apple DTS](https://developer.apple.com/forums/thread/815952?answerId=880880022#880880022)), and that eligibility `false` is Apple's current report, [not a stable region flag](https://developer.apple.com/forums/thread/820699).
